...
| --- | --- | --- |
| **HTTP API** | Axum routes with a generated route inventory, checked by a test (`inventory_matches_route_sources`, run via `cargo test`/`make routes-check` — not part of `cargo check`) that parses the route registrations in source and fails on anything undeclared or stale. Deliberately source-level, not a live-router probe (that would need a fully booted SQLite/NATS `AppState`) — and it can't see two identical `.route(...)` calls within one source file, since parsing collapses duplicates into a set before comparing; that case still panics at router boot instead | [`apps/servers/file_host/src/routes`](./apps/servers/file_host/src/routes), [route inventory](./apps/servers/file_host/docs/route-inventory.md) |
