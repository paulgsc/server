...
pub mod duration {
	...
	fn duration_from_str(s: &str) -> Result<Duration, ParseDurationError> {
		...
		if seconds >= MAX_SECONDS {
			...
			Ok(Duration::try_seconds(seconds).ok_or(ParseDurationError::DurationSeconds { seconds })? + Duration::nanoseconds(nanoseconds.into()))
		}
	}

	pub fn to_string(duration: &Duration) -> String {
		let seconds = duration.num_seconds();
		let nanoseconds = (*duration - Duration::try_seconds(seconds).expect("Seconds in bounds to create Duration from"))
			.num_nanoseconds()
...
mod test {
	...
	fn test_duration_de_success_cases() {
		...
		for (repr, nanos) in durations.into_iter() {
			let wrapper: DurationWrapper = serde_json::from_str(&format!("{{\"duration\": \"{}\"}}", repr)).unwrap();
			assert_eq!(
				Some(nanos),
	...
	fn test_duration_de_failure_cases() {
		...
		for repr in durations.into_iter() {
			assert!(
				serde_json::from_str::<DurationWrapper>(&format!("{{\"duration\": \"{}\"}}", repr)).is_err(),
	...
	fn test_duration_ser_success_cases() {
		let durations = [-200_000_000, 1, 999_999_999_999, 129_000_000_000, 123_456_789];
		...
		for nanos in durations.into_iter() {
			...
			let s = s.unwrap();
			assert_eq!(wrapper, serde_json::from_str(&s).unwrap(), "round trip should return same duration");
		}
	}

	#[test]
	fn standard_base64_de_success_cases() {
		let wrapper: Base64StandardWrapper = serde_json::from_str(r#"{"bytes": "cVhabzk6U21uOkN+MylFWFRJMVFLdEh2MShmVHp9"}"#).unwrap();
		assert_eq!(Some(b"qXZo9:Smn:C~3)EXTI1QKtHv1(fTz}".as_slice()), wrapper.bytes.as_deref());
	}

	#[test]
	fn standard_base64_de_reader_success_cases() {
		let standard: Base64StandardWrapper = serde_json::from_reader(r#"{"bytes": "cVhabzk6U21uOkN+MylFWFRJMVFLdEh2MShmVHp9"}"#.as_bytes()).unwrap();
		assert_eq!(Some(b"qXZo9:Smn:C~3)EXTI1QKtHv1(fTz}".as_slice()), standard.bytes.as_deref());
	}

	#[test]
	fn urlsafe_base64_de_success_cases() {
		let wrapper: Base64URLSafeWrapper = serde_json::from_str(r#"{"bytes": "aGVsbG8gd29ybGQ="}"#).unwrap();
		assert_eq!(Some(b"hello world".as_slice()), wrapper.bytes.as_deref());
	}

	#[test]
	fn urlsafe_base64_de_reader_success_cases() {
		let wrapper: Base64URLSafeWrapper = serde_json::from_reader(r#"{"bytes": "aGVsbG8gd29ybGQ="}"#.as_bytes()).unwrap();
		assert_eq!(Some(b"hello world".as_slice()), wrapper.bytes.as_deref());
	}
	...
	#[test]
	fn num_roundtrip() {
		let wrapper = I64Wrapper { num: Some(i64::MAX) };

		let json_repr = &serde_json::to_string(&wrapper);
		assert!(json_repr.is_ok(), "serialization should succeed");
		assert_eq!(wrapper, serde_json::from_str(&format!("{{\"num\": \"{}\"}}", i64::MAX)).unwrap());
	...
	fn test_empty_wrapper() {
		...
		assert_eq!(Base64URLSafeWrapper { bytes: None }, serde_json::from_str("{}").unwrap());
		assert_eq!(Base64StandardWrapper { bytes: None }, serde_json::from_str("{}").unwrap());
