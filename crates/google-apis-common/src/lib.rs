...
pub trait Delegate: Send {
	...
	/// first place
	fn token(&mut self, e: Box<dyn StdError + Send + Sync>) -> std::result::Result<Option<String>, Box<dyn StdError + Send + Sync>> {
		Err(e)
	}
	...
	/// If you choose to retry after a duration, the duration should be chosen using the
	/// [exponential backoff algorithm](http://en.wikipedia.org/wiki/Exponential_backoff).
	fn http_failure(&mut self, _: &hyper::Response<hyper::body::Body>, _err: Option<serde_json::Value>) -> Retry {
		Retry::Abort
	}
...
impl Display for Error {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		match self {
			...
			Error::UploadSizeLimitExceeded(resource_size, max_size) => writeln!(f, "The media size {} exceeds the maximum allowed upload size of {}", resource_size, max_size),
			Error::MissingAPIKey => {
...
impl<'a> MultiPartReader<'a> {
	...
	pub fn add_part(&mut self, reader: &'a mut (dyn Read + Send), size: u64, mime_type: Mime) -> &mut MultiPartReader<'a> {
		...
		headers.insert(CONTENT_TYPE, hyper::header::HeaderValue::from_str(mime_type.as_ref()).unwrap());
	...
	fn is_depleted(&self) -> bool {
		self.raw_parts.is_empty() && self.current_part.is_none() && self.last_part_boundary.is_none()
	}
		...
		match (self.raw_parts.len(), self.current_part.is_none(), self.last_part_boundary.is_none()) {
			...
			(n, true, _) if n > 0 => {
				...
				(write!(
					...
					BOUNDARY,
					LINE_ENDING,
					headers.iter().map(|(k, v)| format!("{}: {}", k, v.to_str().unwrap())).join(LINE_ENDING),
					LINE_ENDING,
					LINE_ENDING,
				))?;
				c.rewind()?;
			...
			Ok(bytes_read) => {
				if hb < buf.len() && bytes_read == 0 {
					if self.is_last_part() {
						...
						self.last_part_boundary = Some(Cursor::new(format!("{}--{}--{}", LINE_ENDING, BOUNDARY, LINE_ENDING).into_bytes()))
					}
				...
				.await
			{
				Ok(r) => {
					...
					let h: RangeResponseHeader = match headers.get("Range") {
						Some(hh) if r.status() == StatusCode::PERMANENT_REDIRECT => RangeResponseHeader::from_bytes(hh.as_bytes()),
						None | Some(_) => {
			...
			match res {
				Ok(res) => {
					...
					if !reconstructed_result.status().is_success() {
						if let Retry::After(d) = self.delegate.http_failure(&reconstructed_result, json::from_str(&res_body_string).ok()) {
							sleep(d).await;
							continue;
						}
					}
...
mod test_api {
	...
	fn byte_range_from_str() {
		assert_eq!(<Chunk as FromStr>::from_str("2-42"), Ok(Chunk { first: 2, last: 42 }))
	}
	...
	fn test_mime() {
		...
		assert_eq!(Some(BOUNDARY), mime.get_param("boundary").map(|x| x.as_str()));
	}
}
