...
impl<'a> Params<'a> {
	...
	pub fn extend<I: Iterator<Item = (&'a String, IC)>, IC: Into<Cow<'a, str>>>(&mut self, params: I) {
		self.params.extend(params.map(|(k, v)| (k.as_str(), v.into())))
	}

	pub fn get(&self, param_name: &str) -> Option<&str> {
		self.params.iter().find(|(name, _)| name == &param_name).map(|(_, param)| param.as_ref())
	}

	pub fn uri_replacement(&self, url: String, param: &str, from: &str, url_encode: bool) -> String {
		...
		if url_encode {
			...
			let replace_with = self.get(param).expect("to find substitution value in params");
