...
enum JSXState {
	NotInJSX,     // Not in JSX context
	InTagName,    // After < but before tag name is complete
	InAttributes, // Inside a tag after the tag name
	InJSXContent, // Between open and close tags (content)
	InClosingTag, // Inside a closing tag </...>
}
...
impl<'a> Lexer<'a> {
	...
	fn read_jsx_attribute_name(&mut self) -> String {
		let mut attribute_name = String::new();

		// Read the first part which should be a valid identifier
		while let Some(ch) = self.current_char {
		...
		// Handle special cases like hyphenated names or namespaced attributes
		while let Some(ch) = self.current_char {
			if ch == ':' || ch == '-' {
				attribute_name.push(ch);
				self.read_char();

				// Continue reading after colon or hyphen
				while let Some(next_ch) = self.current_char {
					if next_ch.is_ascii_alphanumeric() || next_ch == '_' || next_ch == '$' {
						attribute_name.push(next_ch);
						self.read_char();
					} else {
						break;
					}
	...
	pub fn next_token(&mut self) -> Token {
		// Skip whitespace except in JSX content mode where it might be significant
		if self.jsx_state != JSXState::InJSXContent {
		...
		match self.jsx_state {
			JSXState::NotInJSX => {
				// Regular JavaScript tokenization, but watch for JSX start
				if ch == '<' {
					// Check if this is potentially JSX
					let next_char = self.peek_char();

					// If the next character could be a JSX tag name start
					if let Some(next) = next_char {
						if next.is_alphabetic() || next == '_' || next == '$' || next == '/' {
							if next == '/' {
								// This is a closing tag
								self.jsx_state = JSXState::InClosingTag;
							} else {
								// This is an opening tag
								self.jsx_state = JSXState::InTagName;
								self.jsx_depth += 1;
							}
						}
					}
			...
			JSXState::InTagName => {
				if ch.is_alphabetic() || ch == '_' || ch == '$' {
					...
					// After reading tag name, we're now in attributes state
					self.jsx_state = JSXState::InAttributes;
					return Token::new(TokenType::JSXIdentifier, tag_name, line, column);
				} else {
					// Unexpected character in tag name
					let literal = ch.to_string();
					self.read_char();
					return Token::new(TokenType::Illegal, literal, line, column);
				}
			}
			JSXState::InAttributes => {
				match ch {
					'>' => {
						// End of opening tag
						let literal = ch.to_string();
						self.read_char();
						self.jsx_state = JSXState::InJSXContent;
						return Token::new(TokenType::Gt, literal, line, column);
					}
					'/' => {
						if self.peek_char() == Some('>') {
							...
							if self.jsx_depth == 0 {
								self.jsx_state = JSXState::NotInJSX;
							} else {
								self.jsx_state = JSXState::InJSXContent;
							}

							return Token::new(TokenType::JSXOpeningElementEnd, literal, line, column);
						} else {
							// Just a slash inside attributes
							let literal = ch.to_string();
							self.read_char();
							return Token::new(TokenType::Slash, literal, line, column);
						}
					}
					'{' => {
						// JSX expression in attributes
						let literal = ch.to_string();
						self.read_char();
						// Temporarily switch to JS mode to parse the expression
						self.jsx_state = JSXState::NotInJSX;
						// We'll need to handle returning to prev_state after }
						// This would need a stack of states or special handling
						return Token::new(TokenType::BraceL, literal, line, column);
					}
					'=' => {
						// Attribute equals sign
						let literal = ch.to_string();
						self.read_char();
						return Token::new(TokenType::Eq, literal, line, column);
					}
					'"' | '\'' | '`' => {
						// Attribute string value
						let string_value = self.read_string(ch);
						return Token::new(TokenType::JSXAttributeStringValue, string_value, line, column);
					}
					_ if ch.is_alphabetic() || ch == '_' || ch == '$' => {
						// Attribute name
						let attribute_name = self.read_jsx_attribute_name();
						return Token::new(TokenType::JSXAttributeName, attribute_name, line, column);
					}
					_ => {
						// Other character in attributes
						let literal = ch.to_string();
						self.read_char();
						return Token::new(TokenType::Illegal, literal, line, column);
					}
			...
			JSXState::InJSXContent => {
				if ch == '<' {
					// Could be start of a nested JSX tag or closing tag
					let next_char = self.peek_char();
					...
					if next_char == Some('/') {
						...
						self.read_char(); // Consume '/'
						self.jsx_state = JSXState::InClosingTag;
						return Token::new(TokenType::JSXClosingElementStart, literal, line, column);
					} else {
						// Nested opening tag
						let literal = ch.to_string();
						self.read_char();
						self.jsx_state = JSXState::InTagName;
						self.jsx_depth += 1;
						return Token::new(TokenType::Lt, literal, line, column);
					}
				} else if ch == '{' {
					// JSX expression in content
					let literal = ch.to_string();
					self.read_char();
					// Similar to above - need to handle returning from JS mode
					self.jsx_state = JSXState::NotInJSX;
					return Token::new(TokenType::BraceL, literal, line, column);
				} else {
					// JSX text content
					...
					if !text.trim().is_empty() {
						return Token::new(TokenType::JSXText, text, line, column);
					} else {
