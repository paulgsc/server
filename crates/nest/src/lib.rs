...
pub trait Run {
	...
	fn run<I>(config: Config, handlers: I) -> Self::Future
	where
...
impl Run for ApiBuilder {
	type Future = Pin<Box<dyn Future<Output = Result<()>> + Send + 'static>>;

	// Now the run method accepts handlers
	fn run<I>(config: Config, handlers: I) -> Self::Future
	where
		I: IntoIterator<Item = Box<dyn MultiDbHandler + Send>> + Send + 'static,
	{
		Box::pin(async move {
			let mut api_builder = Self::new(config.clone());
			...
			// Add the dynamic handlers passed into the function
			for handler in handlers {
				api_builder.add_handler(handler);
			}

			api_builder.serve().await?; // Serve the API
