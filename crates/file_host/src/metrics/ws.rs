...
macro_rules! timed_ws_operation {
	($operation_type:expr, $stage:expr, $body:block) => {{
		...
		WS_MESSAGE_PROCESSING_DURATION.with_label_values(&[$operation_type, $stage]).observe(duration);
...
macro_rules! update_connection_state {
	($from_state:expr, $to_state:expr) => {
		// Decrement old state
		match $from_state {
			"active" => WS_CONNECTIONS_ACTIVE.with_label_values(&["active"]).dec(),
			"stale" => WS_CONNECTIONS_ACTIVE.with_label_values(&["stale"]).dec(),
			"disconnected" => WS_CONNECTIONS_ACTIVE.with_label_values(&["disconnected"]).dec(),
			_ => {}
		}

		// Increment new state
		match $to_state {
			"active" => WS_CONNECTIONS_ACTIVE.with_label_values(&["active"]).inc(),
			"stale" => WS_CONNECTIONS_ACTIVE.with_label_values(&["stale"]).inc(),
			"disconnected" => WS_CONNECTIONS_ACTIVE.with_label_values(&["disconnected"]).inc(),
			_ => {}
		}
...
macro_rules! check_invariant {
	($condition:expr, $invariant_type:expr, $description:expr) => {
		if !$condition {
			WS_INVARIANT_VIOLATIONS.with_label_values(&[$invariant_type]).inc();
...
macro_rules! health_check {
	($check_type:expr, $body:block) => {{
		...
		WS_HEALTH_CHECKS.with_label_values(&[$check_type, result_label]).inc();
...
macro_rules! update_resource_usage {
	($resource_type:expr, $value:expr) => {
		WS_RESOURCE_USAGE.with_label_values(&[$resource_type]).set($value);
	};
}
