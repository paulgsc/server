...
mod tests {
    ...
    fn test_yards_parsing() {
        assert_eq!(
            "(15:00 - 1st) (Shotgun) B.Robinson right tackle to ATL 32 for 2 yards (T.Smith, L.David).".parse::<Yards>().unwrap(),
            Yards { value: 2, yard_type: YardType::Gain }
        );
        assert_eq!(
            "(12:39 - 1st) (Shotgun) B.Robinson right end to TB 18 for no gain (T.Smith).".parse::<Yards>().unwrap(),
            Yards { value: 0, yard_type: YardType::NoGain }
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Yards { value: 1, yard_type: YardType::Loss }
        );
    }
    ...
    fn test_invalid_play_description() {
        assert_eq!(
            "Invalid play description".parse::<Yards>().unwrap_err(),
            YardsError::NoYardsInfo
        );
    }
}
