...
mod tests {
    ...
    fn test_parse_run_play() {
        ...
        assert_eq!(PlayType::from_str("B.Mayfield scrambles right end pushed ob at TB 35 for 8 yards (A.Hamilton)."), Ok(PlayType::Run));
    }

    #[test]
    fn test_parse_pass_play() {
        assert_eq!(PlayType::from_str("(8:22 - 2nd) (No Huddle, Shotgun) K.Cousins pass short right to K.Pitts to TB 38 for 8 yards (L.David)."), Ok(PlayType::Pass));
    }

    #[test]
    fn test_parse_sack() {
        assert_eq!(PlayType::from_str("(15:00 - 2nd) K.Cousins sacked at MIN 25 for -7 yards (W.Gholston)."), Ok(PlayType::Sack));
    }
