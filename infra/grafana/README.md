...
|---|---|---|---|
| `dashboard.jsonnet` | First screen: is file_host in a fault state (six conditions, `docs/fault-conditions.md`); what are its traffic, real WS clients, p95 latency, availability and restarts (golden signals, with trend sparklines); are the other services up, and when weren't they (a per-service timeline). Below: traffic/latency/errors, what each service costs the host (CPU and memory vs its limit, disk I/O, disk space), and the WebSocket internals. Collapsed drill-downs: connection lifecycle, rate limiting, the nudge waker, cache/storage. Panel layer: `lib/health-panels`, `lib/overview-panels` (+ `lib/glance` constructors). | `up`, `dependency_up`, `http_requests_total`/`http_request_duration_seconds`, `refusals_total`, `service_info`, `process_start_time_seconds`, `ws_connections`/`ws_connection_lifecycle_total`/`ws_connection_duration_seconds`/`ws_client_connections{client_type="probe"\|...}`, `sqlite_pool_size`/`sqlite_pool_idle`, `rate_limit_decisions_total`/`rate_limit_tokens_available`/`rate_limit_active_clients`/`rate_limit_capacity_per_minute` (file_host, `metrics` facade), `nudge_waker_due_subjects`/`nudge_waker_verdicts_total`/`nudge_waker_enabled` (file_host `nudge::waker`, `metrics` facade), `container_*` (cadvisor) and `namedprocess_*` (process-exporter) for the Metal row, `node_filesystem_*` (node_exporter), `redis_up` (redis_exporter), `gnatsd_varz_*` (nats-exporter), `cache_hits_total`/`cache_misses_total` (some-cache), `probe_success`/`probe_duration_seconds` (blackbox-exporter), `operation_duration_seconds` | #212 (G2/G3), #213 (F1–F5), #214 (C1–C4), #215 (A1–A3) |
| `sys-dashboard.jsonnet` | Is the host itself healthy — CPU, memory, disk, network? | `node_*` (node_exporter) | #212 (G2/G3) |
| `processor.jsonnet` ("WHO DUNNIT") | Mechanical sympathy: is any resource busy (utilisation) or making anything wait (PSI, throttling), who is responsible (ranked per process and per container), and can the process numbers be trusted (CPU ACCOUNTED). Collapsed: the original hang forensics. Panel layer: `lib/sympathy-panels` (+ `lib/glance`), `lib/forensic-panels`. | `namedprocess_*` (process-exporter), `container_*` (cadvisor), `node_*` incl. `node_pressure_*`, `up`, `ALERTS` | #212 (G2/G3) |
