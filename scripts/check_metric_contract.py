...
EXEMPT_EMITTED: dict[str, str] = {
	...
	"nudge_waker_pass_deadline_exceeded_total": "#264 (SLI3): an alert signal, not a graph, for the same reason as nudge_waker_nothing_to_say_total below — its healthy value is zero. What it counts is already visible to a human on the NUDGE row's Waker Pass Duration panel (nudge_waker_last_pass_duration_seconds reaching nudge_waker_pass_deadline_seconds); this series adds a stable name an alert rule can be written against.",
	"nudge_waker_nothing_to_say_total": "#285 (RCM8): an alert signal, not a graph. Its healthy value is zero forever — a non-zero value means a bug (an empty or unreadable catalogue, or an EngagementClass with no StudySelector arm), which is what docs/study-nudge.md says about it in those words. A panel whose healthy state is a flat line at zero is exactly the 'detector that always reports fine' this check's own docstring was written against, and the conditions behind it are already visible on the NUDGE row's outcomes breakdown by label (verdict=storage_error / nothing_to_provision). What this series adds over those labels is a stable name an alert rule can be written against without depending on a label value surviving a refactor — a reader, but not a dashboard one.",
}
