use crate::{cache::DedupError, CacheStore};
...
impl CacheData {
	...
	pub fn from_generic<T: Serialize>(data: &T) -> Result<Self, DedupError> {
	...
	pub fn into_generic<T: for<'de> Deserialize<'de>>(self) -> Result<T, DedupError> {
		match self {
			CacheData::Generic(value) => {
				let result = serde_json::from_value(value)?;
				Ok(result)
			}
			CacheData::Binary { .. } => Err(DedupError::TypeMismatch("Expected generic data, found binary data".to_string())),
		}
	}

	/// Extract binary data
	pub fn into_binary(self) -> Result<(Vec<u8>, Option<String>), DedupError> {
		match self {
			CacheData::Binary { data, content_type } => Ok((data, content_type)),
			CacheData::Generic(_) => Err(DedupError::TypeMismatch("Expected binary data, found generic data".to_string())),
		}
	}
}
...
impl DedupCache {
	...
	pub async fn get_or_fetch<T, F, Fut>(&self, id: &str, fetcher: F) -> Result<(T, bool), DedupError>
	where
		T: Send + Sync + 'static + Serialize + for<'de> Deserialize<'de>,
		F: FnOnce() -> Fut + Send,
		Fut: Future<Output = Result<T, DedupError>> + Send,
	{
	...
	#[instrument(skip(self, fetcher))]
	pub async fn get_or_fetch_with_ttl<T, F, Fut>(&self, id: &str, ttl: Option<u64>, fetcher: F) -> Result<(T, bool), DedupError>
	where
		T: Send + Sync + 'static + Serialize + for<'de> Deserialize<'de>,
		F: FnOnce() -> Fut + Send,
		Fut: Future<Output = Result<T, DedupError>> + Send,
	{
		...
		let once = self
			...
			.await
			.map_err(|e| DedupError::OperationError(format!("Failed to get or insert in active_fetches: {}", e)))?;
		...
		let _ = once
			.set(cache_data)
			.map_err(|_| DedupError::OperationError("Failed to set result in OnceLock".to_string()))?;
	...
	#[instrument(skip(self, fetcher))]
	pub async fn get_or_fetch_binary<F, Fut>(&self, id: &str, fetcher: F) -> Result<((Vec<u8>, Option<String>), bool), DedupError>
	where
		F: FnOnce() -> Fut + Send,
		Fut: Future<Output = Result<(Vec<u8>, Option<String>), DedupError>> + Send,
	{
	...
	#[instrument(skip(self, fetcher))]
	pub async fn get_or_fetch_binary_with_ttl<F, Fut>(&self, id: &str, ttl: Option<u64>, fetcher: F) -> Result<((Vec<u8>, Option<String>), bool), DedupError>
	where
		F: FnOnce() -> Fut + Send,
		Fut: Future<Output = Result<(Vec<u8>, Option<String>), DedupError>> + Send,
	{
	...
	#[instrument(skip(self, fetcher))]
	pub async fn get_or_fetch_unified<F, Fut>(&self, id: &str, fetcher: F) -> Result<(CacheData, bool), DedupError>
	where
		F: FnOnce() -> Fut + Send,
		Fut: Future<Output = Result<CacheData, DedupError>> + Send,
	{
	...
	#[instrument(skip(self, fetcher))]
	pub async fn get_or_fetch_unified_with_ttl<F, Fut>(&self, id: &str, ttl: Option<u64>, fetcher: F) -> Result<(CacheData, bool), DedupError>
	where
		F: FnOnce() -> Fut + Send,
		Fut: Future<Output = Result<CacheData, DedupError>> + Send,
	{
	...
	pub async fn delete(&self, key: &str) -> Result<bool, DedupError> {
	...
	#[instrument(skip(self))]
	pub async fn flush_all(&self) -> Result<u64, DedupError> {
