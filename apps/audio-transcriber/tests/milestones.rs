...
mod milestone_m1 {
    ...
    fn milestone_m1_hotpath_boundary() {
        ...
        use audio_transcriber::hotpath::{
            FixedKnnEngine, FixedNeighborSet, Neighbor, RealTimeAudioBuffer, SpscRingBuffer,
        };
...
mod milestone_m2 {
    ...
    fn milestone_m2_overflow_returns_err() {
        ...
        assert!(
            result.is_err(),
            "overflow must return Err — buffer must not grow or panic"
        );
...
mod milestone_m3 {
    ...
    fn milestone_m3_full_returns_err_immediately() {
        ...
        assert!(
            result.is_err(),
            "full SPSC must return Err immediately, never block"
        );
            ...
            while received < TOTAL {
                match consumer_buf.try_pop() {
                    Some(item) => {
                        assert_eq!(
                            item, expected,
                            "FIFO order violated: expected {expected}, got {item}"
                        );
                        expected += 1;
                        received += 1;
                    }
...
mod milestone_m4 {
    ...
    fn milestone_m4_result_is_stack_allocated() {
        ...
        fn assert_stack_allocated<T: Sized>() {
            ...
            assert!(
                size < 4096,
                "FixedNeighborSet must be stack-sized, got {size} bytes"
            );
        }
